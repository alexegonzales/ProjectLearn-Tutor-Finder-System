<?php
    ...
    if(isset($_POST['submit'])) {
        ...
        $select_email = "SELECT * FROM tutors WHERE email = '$email'";
	    $check_email = mysqli_query($conn, $select_email);

		if($check_email->num_rows > 0) {

			//check status (login attempts)
            $select_status = "SELECT * FROM tutors WHERE status != 0 AND email = '$email'";
			...
			if($check_status->num_rows > 0) {
				...
				//check all
                $select_all = "SELECT * FROM tutors WHERE email = '$email' AND password = '$pass' AND status != 0";
				...
				if($check_all->num_rows > 0) {
                    ...
                    $sql2 = mysqli_query($conn, "UPDATE tutors SET tutor_status = '{$tutor_status}' WHERE unique_id = {$rows['unique_id']}");

					//reset status
					$reset_status = mysqli_query($conn, "UPDATE tutors SET status = '3' WHERE email = '$email'");

                    if($verification_status == "Verified") {
                        //redirect to this page
                        echo "<script>
                            alert('Login Successfully!');
                            document.location.href = 'tutor_page.php';
                            </script>";
					...
					$get_status = mysqli_query($conn, "SELECT * FROM tutors WHERE email = '$_SESSION[email]'");
					...
					$update_status = mysqli_query($conn, "UPDATE tutors SET status = '$new_status' WHERE email = '$_SESSION[email]'");
					...
					if($update_status) {
						...
						if($new_status == 0) {
								...
								$update_lock_date = mysqli_query($conn, "UPDATE tutors SET lock_date = '$time_stamp' WHERE email = '$_SESSION[email]'");
  ...
  <body>
                    ...
                    <div class="col-md-7">
                        ...
                        <div class="card-body">
                            ...
                            <form action="" method="POST" class="login-email">  
                                ...
                                <p class="login-register-text" style="text-align:center;">Don't have an account?<a href="tutor_reg.php"> Register Here</a></p>
                            </form>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        var database_lock_date = "<?php 
                                        $get_lock_date  = mysqli_query($conn, "SELECT * FROM tutors WHERE email = '$_SESSION[email]'");
