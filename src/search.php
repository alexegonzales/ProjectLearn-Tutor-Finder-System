    ...
    <div class="container">
        <div class="row mt-4">
            <nav class="navbar bg-transparent mb-2 navbar-light">
                <div class="container-fluid">
                    ...
                    <form class="d-flex" action="" method="GET">
                        <input class="form-control me-2" name="searchkey" type="search" value="<?php if(isset($_GET['searchkey'])){echo $_GET['searchkey']; } ?>" placeholder="Topic" aria-label="Search" >
            ...
            <?php
                ...
                $sql_select = "SELECT tutors.*, GROUP_CONCAT(topics.topic_name SEPARATOR ', ') 
                AS topic_name  
                FROM tutors
                LEFT JOIN credibility
                ON tutors.id = credibility.sender_id
                LEFT JOIN topics
                ON credibility.sender_id = topics.tutor_id
                WHERE credibility.req_status = 1 AND tutors.id = topics.tutor_id AND CONCAT(firstname, lastname, topic_name) LIKE '%{$searchkey}%' GROUP BY id";

                $sql_select_result = mysqli_query($conn, $sql_select);

                if ($rows = mysqli_num_rows($sql_select_result) > 0) {

                    foreach($sql_select_result as $rows) {
                                    ...
                                    <div class="card">
                                        ...
                                        <div class="card-body">
                                            <table class="table table-borderless">
                                                ...
                                                <tr>
                                                    <td></td>
                                                    <td align="justify" class="info"><div class="btn-view position-absolute bottom-0 end-0 translate-middle"><button type="button" class="btn-modal" data-bs-toggle="modal" data-bs-target="#exampleModal<?php echo $rows['id']?>">view more</button></div></td>
                                                </tr>
                                            </table>
                                        </div>
                        ...
                        echo "<br/><label align='center' class='res'>No result for " . "'" . $_SESSION['searchkey'] . "'" . "</label>";
                    }
