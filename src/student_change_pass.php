    ...
    && isset($_POST['confirm_newpassword'])) {
        ...
        $npassPattern = "/^\S*(?=\S{8,})(?=\S*[a-z])(?=\S*[A-Z])(?=\S*[\d])\S*$/i";
            ...
            if (preg_match($npassPattern, $newpass) == 0) {
                ...
                header("Location: student_change_pass.php?error=Invalid new password");
    
            }elseif ($newpass !== $c_newpass) {

                header("Location: student_change_pass.php?error=The confirmation password does not match");
                exit();
            ...
            $sql = "SELECT password FROM students WHERE id='$id' AND password='$hash_oldpass'";

            $result = mysqli_query($conn, $sql);

            if(mysqli_num_rows($result) === 1){
                
                $sql_2 = "UPDATE students SET password='$hash_newpass'WHERE id='$id'";

                mysqli_query($conn, $sql_2);

                header("Location: student_login.php?Your password has been changed successfully");
                exit();

            }else {
                header("Location: student_change_pass.php?error=Current password does not match");
                exit();
            }
